...
// Credit to https://github.com/amirmehdizadeh/JalaliCalendar
...
public class JalaliCalendar extends Calendar {
    ...
    static final int LEAST_MAX_VALUES[] = { CE, // ERA
            292269054, // YEAR
            ESFAND, // MONTH
            52, // WEEK_OF_YEAR
            4, // WEEK_OF_MONTH
    ...
    public static YearMonthDate jalaliToGregorian(YearMonthDate jalali) {
        ...
        gregorianYear = 1600
                + 400 * (int) Math.floor(gregorianDayNo / 146097); /* 146097 = 365*400 + 400/4 - 400/100 + 400/400 */
    ...
    public static int dayOfWeek(YearMonthDate yearMonthDate) {
        ...
        Calendar cal = new GregorianCalendar(yearMonthDate.getYear(), yearMonthDate.getMonth(),
    ...
    public static boolean isLeepYear(int year) {
        ...
        if ((year % 33 == 1 || year % 33 == 5 || year % 33 == 9 || year % 33 == 13 || year % 33 == 17 || year % 33 == 22
    ...
    protected void computeTime() {
        ...
        if (!isTimeSet && !isTimeSeted) {
            ...
            YearMonthDate yearMonthDate = jalaliToGregorian(
                    new YearMonthDate(internalGet(YEAR), internalGet(MONTH), internalGet(DAY_OF_MONTH)));
    ...
    public void set(int field, int value) {
        switch (field) {
            ...
            case ZONE_OFFSET:
            case DST_OFFSET: {
                if (isSet(YEAR) && isSet(MONTH) && isSet(DATE) && isSet(HOUR) && isSet(HOUR_OF_DAY) && isSet(MINUTE)
                        && isSet(SECOND) && isSet(MILLISECOND)) {
                    cal = new GregorianCalendar();
                    YearMonthDate yearMonthDate = jalaliToGregorian(
                            new YearMonthDate(internalGet(YEAR), internalGet(MONTH), internalGet(DATE)));
                    cal.set(yearMonthDate.getYear(), yearMonthDate.getMonth(), yearMonthDate.getDate(),
                            internalGet(HOUR_OF_DAY), internalGet(MINUTE), internalGet(SECOND));
                    cal.set(field, value);
    ...
    public void add(int field, int amount) {
        ...
        if (field == MONTH) {
            ...
            Calendar gc = new GregorianCalendar(yearMonthDate.getYear(), yearMonthDate.getMonth(),
                    yearMonthDate.getDate(), get(HOUR_OF_DAY), get(MINUTE), get(SECOND));
            gc.add(field, amount);
